...
bool ModbusMaster::getResultMsg(uint8_t result)
{	
  String tmpstr2;
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  if (_preTransmission)
  {
    Serial.println("Entrando en modo de Pretransmision...");
	_preTransmission();
  }
 ...
 debugSerialPort.println();
  ...
  Serial.println("Trama Modbus enviada por el puerto Serial2");
  if (_postTransmission)
  {
    Serial.println("Activando modo de Postransmision...");
	_postTransmission();
  }
  ...
  if (!u8MBStatus)
  {
	...
	for(int j=0; j<u8ModbusADUSize; j++)
	{
		Serial.print(u8ModbusADU[j]);
		Serial.print(", ");
	}
  }
